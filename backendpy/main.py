...
async def send_alert_email(subject: str, message: str):
    ...
    if not EMAIL_SENDER or not EMAIL_PASS:
        print("⚠️ Email credentials not set in environment. Cannot send email.")
        return
...
async def get_devices():
    # The frontend expects data in this specific nested format
...
async def set_device_status(device_id: str, payload: TogglePayload):
    ...
    try:
        # Create a client instance for this single action
        ...
        # Update our local state for immediate UI feedback on the frontend
...
async def save_limits(device_id: str, payload: LimitsPayload):
    if device_id in device_master_list:
        # Ensure the 'limits' key exists before trying to update it
    ...
    while True:
        ...
        try:
            ...
            for device in devices:
                ...
                # Update our master list with the latest data from the server
